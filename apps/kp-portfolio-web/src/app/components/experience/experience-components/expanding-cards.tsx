...
import useCardMouseHover from '@/hooks/useCardMouseHover';
...
export function ExpandingCards({revealFunc}:{revealFunc: () => void}) {
  ...
  const activeIndex = useCardMouseHover(toggleRefs)
        ...
        <div className={styles['expanding-cards-wrapper']}>
          <div className={styles['expanding-cards-container']}>
            {EXPERIENCE_CARDS.map((exp, index) => (
              <div ref={(el) => { toggleRefs.current[index] = el; }} className={styles['expanding-card'] + ` toggle ${activeIndex === index ? styles.active : ''}`}>
