...
export function ToOtherExp({body, toExpFunc}:{body: string; toExpFunc: () => void}){
  ...
  return (
    <Reveal visibleV={{filter: 'blur(0)', opacity: 1 }} hiddenV={{filter: 'blur(20px)', opacity: 0 }} className="py-24 px-4 bg-slate-50">
      <div className="container mx-auto text-center max-w-3xl">
        <Reveal tag={'h2'} delay={0.2} className="text-3xl md:text-5xl font-bold mb-6 leading-tight">{t('experience.to.other.exp.title')}</Reveal>
        <Reveal delay={0.2}><p className="mb-10 text-xl" dangerouslySetInnerHTML={{__html: t(body)}}/></Reveal>
