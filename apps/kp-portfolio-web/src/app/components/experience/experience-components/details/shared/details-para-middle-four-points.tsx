    ...
    <div className="mx-auto max-w-6xl py-20">
      <div className="text-center mb-20">
        ...
        <Reveal duration={0.5} delay={0.1}>
          <p className="text-xl max-w-4xl mx-auto mb-8" dangerouslySetInnerHTML={{__html: t(description)}}>
          </p>
        </Reveal>
      </div>
        ...
        {contents.map((feature) => (
          <Reveal duration={0.5} delay={0.1} key={feature.title} className="flex flex-col space-y-4">
            <div className="flex items-center gap-4">
              ...
              <h3 className="text-2xl font-semibold text-green-900">{t(feature.title)}</h3>
            </div>
            <p className="text-lg pl-12">{t(feature.description)}</p>
          </Reveal>
        ))}
      </div>
    </div>
