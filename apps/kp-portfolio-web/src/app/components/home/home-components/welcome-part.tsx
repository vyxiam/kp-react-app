...
export function HomeWelcome() {
  ...
  const showFirstStatus = useTimeout(1.2)
  const showSecondStatus = useTimeout(1.5)
  return (
    <section className='flex flex-col 2xl:flex-row'>
      ...
      <div className='flex flex-col items-center'>
        <Reveal><GlowingCard title={'Full stack'} img={VACODE} className={'mt-20 2xl:mt-40'}/></Reveal>
        <StaggeredFlipInTitle text={'Full-stack'} className={'mt-12 text-3xl'}/>
        <div className='flex flex-col items-center gap-3 mt-4 text-4xl'>
          {showFirstStatus &&
            (<GlitchTitle
              tag={'h3'} randomColor={'text-purple-500'}
              className={'text-blue-200'} title={'Software Developer'} />)}
          {showSecondStatus &&
            (<GlitchTitle
              tag={'h3'} randomColor={'text-purple-500'}
              className={'text-blue-200'} title={'Web Developer'} />)}
        </div>
      </div>
    </section>
  );
